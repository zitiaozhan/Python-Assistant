...
> 当前状态：已实现 **Web UI 对话 + Markdown 渲染（含表格） + Agent 工具调用 + 技能系统 + Token 统计 + 系统提示词变量 + 配置管理 + 手机扫码远程访问 + 上下文压缩**。Agent 能自主决定是否调用工具或技能，在本地真实执行，文本流式输出，并在每轮对话后报告 Token 消耗。手机端可通过 QR 码扫描连接到同一对话会话，实现双向消息同步。支持长期任务运行，上下文过长时自动压缩历史对话。
...
当对话历史过长时，Agent 会自动压缩上下文，以支持长期任务运行：
...
Agent 还可以调用 `compress_context` 工具主动触发压缩：
- 任务阶段性完成时保存进展
- 认为对话历史过长需要清理时
- 需要切换新任务主题时
...
- [x] **Markdown 表格渲染**（支持对齐方式、悬停高亮、横向滚动、聊天消息 + Skill 编辑器）
- [x] **UI 现代化**（新设计语言：气泡样式、卡片阴影、动画、QR 码弹窗）
- [x] **手机扫码访问**（广播 WebSocket + 共享会话历史 + 历史回放 + 移动端专属页面）
- [x] **上下文压缩**（AI 总结历史 + 自动触发 + 主动触发、支持长期任务运行）
